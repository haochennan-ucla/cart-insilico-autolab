...
BRANCH = "main"
PACKAGE_NAME = "cart_autolab_offline_reproducibility"
KEY_CHECKSUMS = [
    "logs/pytest.log",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build the offline reproducibility release package.")
    parser.add_argument(
    ...
    release_root = repo_root / "offline_release"
    ...
    zip_path = dist_dir / f"cart_autolab_offline_reproducibility_{commit_short}.zip"
    sha_path = dist_dir / f"cart_autolab_offline_reproducibility_{commit_short}.sha256"
    if zip_path.exists():
        zip_path.unlink()
    if sha_path.exists():
        sha_path.unlink()
...
def write_package_readme(path: Path, *, commit: str, github_actions_status: str, github_actions_note: str) -> None:
    path.write_text(
        f"""# Offline Reproducibility Package

This is an offline reproducibility package generated from `{BRANCH}` at commit `{commit}`.
...
def build_known_issues(
    ...
    *,
    ...
    lines = [
        ...
        "- Live LLM execution requires provider credentials and is not required for the offline reproducibility package.",
