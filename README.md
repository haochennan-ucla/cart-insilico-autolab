...
```bash
git clone https://github.com/haochennan-ucla/cart-insilico-autolab.git
cd cart-insilico-autolab
python -m venv .venv
.venv\Scripts\activate
pip install -e .[dev]
```

Naming note: the installable project name is `cart-insilico-autolab`, and the Python package imported by the code is `cart_autolab`. The checkout directory depends on how the repository is cloned; for the GitHub command above it is `cart-insilico-autolab`.
